...
## Testing Guidelines
Automated tests live under `tests/` and run through `npm test`; keep parser and utility regressions there when they do not need a VS Code host. Use the sample CSV to manually verify load, edit, add/remove column and row interactions, and the Save button round-trip in the Extension Development Host. When working on parsing, also test files with different newline styles to confirm `Papa.parse` integration.
